          ...
          <div className="grid grid-cols-2 md:grid-cols-4 auto-rows-[120px] gap-3">
            ...
            {(["premium", "event", "marketplace", "squadron"] as const).map(k => (
              <div
                ...
                onClick={() => reveal(k)}
              >
                {revealed[k] ? <span className="text-lg md:text-xl lg:text-3xl">{yesNo(vehicle?.[`is_${k}`])}</span> : <span className="text-lg md:text-xl lg:text-3xl uppercase tracking-wider text-[#b9c5e1] text-wrap">{k === "marketplace" ? "Market" : k === "squadron" ? "Squad" : k.charAt(0).toUpperCase() + k.slice(1)}</span>}
              </div>
            ))}
          </div>
